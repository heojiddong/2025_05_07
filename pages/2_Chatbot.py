...
# 세션 상태 초기화
if "chat3_messages" not in st.session_state:
    st.session_state.chat3_messages = []

if "chat3_visible" not in st.session_state:
    st.session_state.chat3_visible = False
...
if "api_key" in st.session_state and st.session_state.api_key:
    ...
    def create_library_assistant():
        assistant = openai.beta.assistants.create(
            name="Pukyong Library Chatbot",
            instructions=f"""
You are a chatbot for the National Pukyong University Library.
Answer user questions only using the official regulation document below.

--- 도서관 규정 시작 ---
{LIBRARY_RULES}
--- 도서관 규정 끝 ---

Always answer based strictly on the above regulation content.
            """,
            model="gpt-4.1-mini"
        )
        return assistant.id

    if "library_assistant_id" not in st.session_state:
        st.session_state.library_assistant_id = create_library_assistant()
    ...
    # 사용자 입력
    user_input = st.chat_input("도서관 규정에 대해 질문해 주세요...")

    if user_input:
        st.session_state.chat3_messages.append({"role": "user", "content": user_input})
        ...
        run = openai.beta.threads.runs.create(
            thread_id=thread.id,
            assistant_id=st.session_state.library_assistant_id,
        )

        with st.spinner("도서관 규정을 확인 중..."):
            while True:
        ...
        for msg in reversed(messages.data):
            if msg.role == "assistant":
                ...
                st.session_state.chat3_messages.append({"role": "assistant", "content": reply})
                break

        st.session_state.chat3_visible = True

    # 출력
    if st.session_state.chat3_visible:
        for msg in st.session_state.chat3_messages:
            with st.chat_message(msg["role"]):
                st.markdown(msg["content"])

else:
    st.error("❌ API 키가 설정되지 않았습니다. 홈 화면에서 먼저 입력해 주세요.")
